...
typedef __gun_cxx::__sso_string string;
#endif
...
void shuffle(int nbuckets, int argc, char* argv[])
{
	...
	for (int i = 1; i < argc; ++i)
	{
		...
		processOneInput(argv[i], boost::bind(&Shuffler::output, &shuffler, _1, _2));

	}
	std::cout << "shuffling done" << std::endl;
}

std::unordered_map<string, int64_t> read_shared(int idx, int nbuckets)
{
	std::unordered_map<string, int64_t> queries;
	char buf[256];
...
void reduce(const int nbuckets)
{
	for (int i = 0; i < nbuckets; ++i)
	{
		std::unordered_map<string, int64_t> queries(read_shared(i, nbuckets));
		...
		char buf[256];
		snprintf(buf, sizeof buf,, "count-%5d-of-%5d", i, nbuckets);
